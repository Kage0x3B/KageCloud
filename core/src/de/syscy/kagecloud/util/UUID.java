...
public final class UUID implements Serializable, Comparable<UUID> {
	...
	public int compareTo(UUID uuid) {
		...
		if(mostSigBits != uuid.mostSigBits) {
			return mostSigBits < uuid.mostSigBits ? -1 : 1;
		}
		assert mostSigBits == uuid.mostSigBits;
		if(leastSigBits != uuid.leastSigBits) {
			return leastSigBits < uuid.leastSigBits ? -1 : 1;
		}
		assert leastSigBits == uuid.leastSigBits;
		return 0;
	}
	...
	private static long peekLong(byte[] src, int offset, ByteOrder order) {
	...
	public static class UUIDSerializer extends Serializer<UUID> {
		...
		public void write(Kryo kryo, Output output, UUID uuid) {
			output.writeLong(uuid.mostSigBits);
			output.writeLong(uuid.leastSigBits);
		}
	}
}